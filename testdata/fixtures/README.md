...
It is local harness documentation, not a second protocol source of truth. `zmux-spec` remains authoritative for fixture format and expected behavior.
...
- `wire_invalid.ndjson`
  ...
  - invalid-frame subsets are also covered by `invalid_frame_fixtures_test.go` and `invalid_frame_fixtures_cases_test.go`
- `state_cases.ndjson`
